...
extern struct PID Control_left1;                                               //右轮PID参数，适于新电机4096
extern struct PID Control_right1;                                              //右轮PID参数，适于新电机4096
...
{    
    ...
    {  
        ...
        IN7H;
	      IN8L;
        Control_right1.OwenValue=(val2-10000);                               //PID调节的目标编码数
    }
    else if(val2<10000)
    {  
        /* motor A 反转*/
        ...
        IN7L;
	      IN8H;
        Control_right1.OwenValue=(10000-val2);                               //PID调节的目标编码数	 
    }	
    else
    {
        //GPIO_SetBits(GPIOC, GPIO_Pin_2);	
...
void car_control(float rightspeed,float leftspeed,float rightspeed1,float leftspeed1)//小车速度转化和控制函数
{
    ...
    int right_speed=(int)k2*rightspeed;                                      //将从串口接收到的速度转换成实际控制小车的速度？还是PWM？
    ...
    LeftMovingSpeedW(left_speed+10000);                                      //电机B
	  RightMovingSpeedW1(right_speed1+10000);                                  //电机C
    LeftMovingSpeedW1(left_speed1+10000);                                    //电机D
}
