...
/******************** PID参数 **********************/
//结构体定义在pid.h
extern struct PID Control_left;                                    //左前轮A的PID参数
extern struct PID Control_right;                                   //右前轮B的PID参数
...
u8 Speed_Buffer_Underlabeling = 0;                                 //全局变量，每执行一次TIM6中断函数，\
                                                                   //自加一次，CNT_Buffer数组移位一次，直到达到最大值清零
...
//          }
            ...
            /********************** 储存脉冲数 *********************/           
            Odometer_R_CNT=(float)temp_cnt1;                          //将脉冲数以全局变量的方式共享给odometer函数
