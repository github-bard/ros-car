...
void RightMovingSpeedW(int val)
{    
    if(val>0)
    {   
        IN1H;
	      IN2L;
        Control_right.OwenValue=(val);                                         //PID调节的目标编码数
    }
    else if(val<0)
    {	
        IN1L;
...
/****************** 左前轮方向和速度控制函数 *******************/
void LeftMovingSpeedW(int val)                                                //左前轮方向和速度控制函数
{     
    if(val>0)
    {  	
        IN3L;
	      IN4H;
        Control_left.OwenValue=(val);                                         //PID调节的目标编码数			
    }
    else if(val<0)
    {  
        IN3H;
...
/****************** 左后轮方向和速度控制函数 *******************/
void LeftMovingSpeedW1(int val)                                              //左后轮方向和速度控制函数
{     
    if(val>0)
    {
        IN5L;
	      IN6H;
        Control_left1.OwenValue=(val);                                       //PID调节的目标编码数			
    }
    else if(val<0)
    {
        IN5H;
...
/****************** 右后轮方向和速度控制函数 *******************/
void RightMovingSpeedW1(int val)
{    
    if(val>0)
    {  
        IN7H;
	      IN8L;
        Control_right1.OwenValue=(val);                                      //PID调节的目标编码数
    }
    else if(val<0)
    {	
        IN7L;
...
{
    float k2=17;//=17.179;                                                   //速度转换比例,转/分钟	                                                                                 
    int right_speed=(int)k2*rightspeed;                                      //将从串口接收到的是预期的每个轮子的线速度
    int left_speed=(int)k2*leftspeed;																			 	 //为什么要做强制数据类型转换? 
	  int left_speed1=(int)k2*leftspeed1;                                      //修改了类型unsigned int
	  int right_speed1=(int)k2*rightspeed1; 
	
		RightMovingSpeedW(right_speed);                                          //电机A
    LeftMovingSpeedW(left_speed);                                            //电机B
	  LeftMovingSpeedW1(left_speed1);                                          //电机C
	  RightMovingSpeedW1(right_speed1);                                        //电机D
}
