...
}rightdata,leftdata,leftdata1,rightdata1; 
...
//	bit15：接收完成标志    
//	bit14：接收到0x0D标志     
//	bit13~0：接收到的有效数据个数，填满相当于十进制的8191
// serial_rec：每次接收串口一个字节的数据
...
void USART3_Handle(void)
{
	  ...
	  u8 i=0;
        ...
        for(i=0;i<3;i++)
        {
            USART_SendData(USART3,0x00);	                                        //连发3组0x00，清空数据？？？
        ...
        for(i=0;i<4;i++)
        {
            ...
            leftdata.recieve_char[i]   = USART_RX_BUF[i+4];
						leftdata1.recieve_char[i]  = USART_RX_BUF[i+8];
						rightdata1.recieve_char[i] = USART_RX_BUF[i+12];
        }
        			
				rightdata.recieve_float=rightdata.recieve_float*100;                      //乘100只是为了让现象更明显，无其它含义 
        leftdata.recieve_float=leftdata.recieve_float*100;         	                   	 
        leftdata1.recieve_float=leftdata1.recieve_float*100;                              
				rightdata1.recieve_float=rightdata1.recieve_float*100;
