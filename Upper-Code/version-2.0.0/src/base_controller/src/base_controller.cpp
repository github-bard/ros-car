...
using std::vector;
/*****************************************************************************/
float ratio = 10.0f ;   //转速转换比例，执行速度调整比例
float D = 0.200000f ;    //两轮间距，单位是m
float linearx_temp=0,lineary_temp=0,angular_temp=0;//暂存的线速度和角速度
/****************************************************/
unsigned char data_terminal0=0x0d;  //“/r"字符
unsigned char data_terminal1=0x0a;  //“/n"字符
unsigned char speed_data[18]={0};   //要发给串口的数据
...
}right_speed_data,left_speed_data,left1_speed_data,right1_speed_data,position_x,position_y,oriention,vel_linear,vel_angular;
/************************************************************/
void callback(const geometry_msgs::Twist &cmd_input)//订阅/cmd_vel主题回调函数
{
    string port("/dev/ttyUSB0");    //小车串口号
    unsigned long baud = 115200;    //小车串口波特率
    serial::Serial my_serial(port, baud, serial::Timeout::simpleTimeout(300)); //配置串口
 
    angular_temp = cmd_input.angular.z ;//获取/cmd_vel的角速度,rad/s
    linearx_temp = cmd_input.linear.x ;//获取/cmd_vel的线速度.m/s
    lineary_temp = cmd_input.linear.y ;
    //将转换好的小车速度分量为左右轮速度,输入时角速度带正负
    right_speed_data.d  = -linearx_temp - lineary_temp - angular_temp;
    left_speed_data.d   = +linearx_temp - lineary_temp - angular_temp;
    left1_speed_data.d  = +linearx_temp + lineary_temp - angular_temp;
    right1_speed_data.d = -linearx_temp + lineary_temp - angular_temp;
  ...
  //  left_speed_data.d*=ratio;   //放大１０００倍，mm/s
    ...
    for(int i=0;i<4;i++)    //将左右轮速度存入数组中发送给串口
    {
        ...
        speed_data[i+8]=left1_speed_data.data[i];
        speed_data[i+12]=right1_speed_data.data[i];
    }
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
      ...
      loop_rate.sleep();//周期休眠
        ...
        //程序周期性调用
        ros::spin();  //callback函数必须处理所有问题时，才可以用到
    }
    return 0;
}
